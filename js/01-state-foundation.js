...
/* v907: the JS build stamp. The release sed bump rewrites the version in BOTH
...
function stripCadenceAndTime(text) {
  ...
  // (v907: "three times a week" in a title left a garbled 7-word fallback
...
function sanitizeTierText(str, titleFallback, opts) {
  ...
  // v907: durations are ALLOWED in tier text. They are the one honest scaling
  ...
  // 9-word ceiling (up from 7, v907). Durations and scope qualifiers are
...
function migrateState() {
  ...
  if (state.action.primaryAction && typeof stripCadenceAndTime === 'function') {
    ...
    // v907: title and howToStart are NO LONGER stripped. The strip garbled
